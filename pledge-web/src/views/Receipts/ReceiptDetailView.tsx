...
interface ReceiptDetailViewProps {
    receiptId: string;
    onClose?: () => void; // Optional: if provided, we show a back/close button suited for modal
}
...
export const ReceiptDetailView: React.FC<ReceiptDetailViewProps> = ({ receiptId, onClose }) => {
        ...
        return (
            <div className="max-w-md mx-auto py-12 px-4 animate-slide-up">
                <div className="bg-surface p-8 rounded-[2.5rem] border border-border text-center space-y-4 shadow-xl">
                    ...
                    {onClose ? (
                        <button onClick={onClose} className="block w-full text-center text-sm font-bold text-foreground underline underline-offset-4 hover:text-accent">Close</button>
                    ) : (
    ...
    return (
        <div className="animate-fade-in w-full max-w-5xl mx-auto">
             {onClose ? (
                <button onClick={onClose} className="inline-flex items-center space-x-2 text-muted hover:text-foreground transition-colors mb-6 group">
                     <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
                    <span className="text-xs font-bold uppercase tracking-widest">Close Preview</span>
                </button>
             ): (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border border-border p-8 rounded-[2rem] bg-background/50">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    {senderAuth?.email && <div className="text-xs text-muted font-medium">{senderAuth.email}</div>}
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex flex-col gap-0.5">
                                    ...
                                    {receiverAuth?.email && <div className="text-xs text-muted font-medium">{receiverAuth.email}</div>}
